...
def convert_examples_to_features(js,sp,args):
    ...
    source_tokens = code.split()
    source_ids = sp.encode(code, out_type=int)
    source_ids = [0] + source_ids[: (args.block_size - 2)] + [2]
    src_lengths = len(source_ids)
    padding_length = args.block_size - src_lengths
    source_ids+=[1]*padding_length
    prev_output_tokens = np.roll(source_ids, 1)
    # TODO fix decoder attention mask 
    # Cancel
    prev_output_tokens[0] = 0
    return InputFeatures(source_tokens,source_ids,src_lengths,prev_output_tokens,js['idx'],js['target'])
...
class TextDataset(Dataset):
    def __init__(self, sp, args, file_path=None):
        ...
        if 'train' in file_path:
            for idx, example in enumerate(self.examples[:3]):
                    ...
                    logger.info("input_lengths: {}".format(example.input_lengths))
                    logger.info("prev_output_tokens: {}".format(' '.join(map(str, example.prev_output_tokens))))
    ...
    def __getitem__(self, i):       
        return torch.tensor(self.examples[i].input_ids),torch.tensor(self.examples[i].label), \
               torch.tensor(self.examples[i].input_lengths),torch.tensor(self.examples[i].prev_output_tokens)
...
def train(args, train_dataset, model, sp):
    ...
    best_mrr=0.0
    best_acc=0.0
    # model.resize_token_embeddings(len(sp))
    model.zero_grad()
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            inputs_lengths=batch[2].to(args.device) 
            prev_tokens=batch[3].to(args.device) 

            model.train()
            loss,logits = model(input_ids=inputs,inputs_lengths=inputs_lengths,prev_tokens=prev_tokens,labels=labels)
...
def evaluate(args, model, sp,eval_when_training=False, eval_data_file=""):
    ...
    for batch in eval_dataloader:
        ...
        prev_tokens=batch[3].to(args.device) 
        with torch.no_grad():
            lm_loss,logit = model(input_ids=inputs,inputs_lengths=inputs_lengths,prev_tokens=prev_tokens,labels=label)
...
def main():
    ...
    sp = spm.SentencePieceProcessor(model_file=args.tokenizer_name)
    if args.block_size <= 0:
        args.block_size = 510  # Our input block size will be the max possible for the model
    ...
    model=Model(model,config,sp,args)
