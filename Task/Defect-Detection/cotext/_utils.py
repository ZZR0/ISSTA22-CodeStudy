...
def convert_defect_examples_to_features(item):
    ...
    if args.model_type in ['t5', 'codet5'] and args.add_task_prefix:
        ...
        source_str = example.source
    code_ids = tokenizer.encode(source_str).ids
    code_ids = code_ids[:args.max_source_length-1] + [tokenizer.token_to_id("</s>")]
    code_ids = code_ids + [tokenizer.token_to_id("<pad>")] * (args.max_source_length-len(code_ids))
