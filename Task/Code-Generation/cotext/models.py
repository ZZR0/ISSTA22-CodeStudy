...
class POJModel(nn.Module):
    def __init__(self, encoder, config, tokenizer, args):
        super(POJModel, self).__init__()
        self.encoder = encoder
        self.config = config
        self.tokenizer = tokenizer
        self.classifier = RobertaClassificationHead(config)
    ...
    def forward(self, input_ids=None,p_input_ids=None,n_input_ids=None,labels=None): 
        ...
        if self.args.model_type == 'codet5':
            ...
            outputs = self.get_bart_vec(input_ids)
        elif self.args.model_type == 'roberta':
            outputs = self.get_roberta_vec(input_ids)
...
class SearchModel(nn.Module):
    ...
    def forward(self, code_ids=None, nl_ids=None, return_vec=False):
        bs=code_ids.shape[0]
        source_ids=torch.cat((code_ids,nl_ids),0)
