...
class Seq2Seq(nn.Module):
    ...
    """
    def __init__(self,model, encoder,decoder,embedding,config,beam_size=None,max_length=None,sos_id=None,eos_id=None):
    ...
    def forward(self, source_ids=None,source_mask=None,target_ids=None,target_mask=None,args=None):   
        ...
        if target_ids is not None:  
            ...
            # tgt_embeddings = self.embedding(target_ids).permute([1,0,2]).contiguous() # L * B * D
            tgt_embeddings = self.embedding(target_ids) # L * B * D
            out = self.decoder(inputs_embeds=tgt_embeddings,
                               encoder_hidden_states=encoder_output,
                               attention_mask=target_mask)[0] # L * B * D
            # hidden_states = torch.tanh(self.dense(out)).permute([1,0,2]).contiguous() # B * L * D
            # hidden_states = torch.tanh(self.dense(out)) # B * L * D
            hidden_states = out
            ...
            for i in range(source_ids.shape[0]): # B
                context=encoder_output[i:i+1] # 1 * L * D
                context_mask=source_mask[i:i+1,:] # 1 * L
                beam = Beam(self.beam_size,self.sos_id,self.eos_id)
                input_ids=beam.getCurrentState() # beam * 1
                context=context.repeat(self.beam_size, 1,1) # beam * L * D
                context_mask=context_mask.repeat(self.beam_size,1) # beam * L
                for _ in range(self.max_length): 
                    if beam.done():
                        break
                    attn_mask=-1e4 *(1-self.bias[:input_ids.shape[1],:input_ids.shape[1]])
                    # tgt_embeddings = self.embedding(input_ids).permute([1,0,2]).contiguous() # 1 * beam * D
                    tgt_embeddings = self.embedding(input_ids) # beam * 1 * D

                    out = self.decoder(inputs_embeds=tgt_embeddings,
                               encoder_hidden_states=context)[0] # beam * 1 * D
