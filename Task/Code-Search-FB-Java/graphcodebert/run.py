...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None):
        self.args = args
        cached_path = os.path.join(args.output_dir, file_path.split("/")[-1].split(".")[0]+"_cached.pkl")
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            loss,code_vec,nl_vec = model(code_inputs=code_inputs,attn_mask=attn_mask,position_idx=position_idx,nl_inputs=nl_inputs)
...
def evaluate(args, model, tokenizer,eval_when_training=False, eval_file=None, steps="-1"):
    ...
    for batch in eval_dataloader:
        ...
        nl_inputs = batch[3].to(args.device)
        with torch.no_grad():
            lm_loss,code_vec,nl_vec = model(code_inputs=code_inputs,attn_mask=attn_mask,position_idx=position_idx,nl_inputs=nl_inputs)
