...
def convert_search_examples_to_features(item):
    ...
    code_ids = tokenizer.encode(code_str, max_length=args.max_source_length, padding='max_length', truncation=True)
    nl_ids = tokenizer.encode(nl_str, max_length=args.max_source_length, padding='max_length', truncation=True)
    assert code_ids.count(tokenizer.eos_token_id) == 1
    assert code_ids.count(tokenizer.eos_token_id) == 1

    return SearchInputFeatures(example_index, code_ids, nl_ids, example.url)

def convert_poj_examples_to_features(item):
    example, example_index, tokenizer, args = item
    if args.model_type in ['t5', 'codet5'] and args.add_task_prefix:
        source_str = "{}: {}".format(args.task, example.source)
    else:
...
def read_poj_examples(filename, data_num):
    """Read examples from filename."""
    examples = []
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
