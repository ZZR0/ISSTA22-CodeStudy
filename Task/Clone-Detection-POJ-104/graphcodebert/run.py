...
def convert_examples_to_features(js,tokenizer,args):
    ...
    return InputFeatures(source_tokens,source_ids,position_idx,dfg_to_code,dfg_to_dfg,js['index'],int(js['label']))
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            position_idx = batch[0][1].to(args.device) 
            attn_mask = batch[0][2].to(args.device)   
            p_inputs = batch[1][0].to(args.device)
            p_position_idx = batch[1][1].to(args.device) 
            p_attn_mask = batch[1][2].to(args.device)
            n_inputs = batch[2][0].to(args.device)
            n_position_idx = batch[2][1].to(args.device) 
            n_attn_mask = batch[2][2].to(args.device)
            labels = batch[3].to(args.device)

            model.train()
            loss,vec = model(inputs,position_idx,attn_mask,
                             p_inputs,p_position_idx,p_attn_mask,
                             n_inputs,n_position_idx,n_attn_mask,
                             labels)
...
def evaluate(args, model, tokenizer,eval_when_training=False, eval_data_file=""):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            lm_loss,vec = model(inputs,position_idx,attn_mask,
                                p_inputs,p_position_idx,p_attn_mask,
                                n_inputs,n_position_idx,n_attn_mask,
                                label)
