...
def convert_examples_to_features(js,tokenizer,args):
    ...
    source_ids = tokenizer.encode(code, out_type=int)
    ...
    return InputFeatures(source_tokens,source_ids,src_lengths,prev_output_tokens,js['index'],int(js['label']))
...
class TextDataset(Dataset):
    ...
    def get_feature(self, example):
        return (torch.tensor(example.input_ids), 
                torch.tensor(example.prev_output_tokens), 
                torch.tensor(example.input_lengths))
    ...
    def __getitem__(self, i):   
        ...
        return (self.get_feature(self.examples[i]),self.get_feature(p_example),
                self.get_feature(n_example),torch.tensor(label))
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
        logger.info("  %s = %s", "Epoch", 0)
        results = evaluate(args, model, tokenizer, eval_when_training=True, eval_data_file=args.test_data_file)
        for key, value in results.items():
            logger.info("Testing:  %s = %s", key, round(value,4))
        results = evaluate(args, model, tokenizer, eval_when_training=True, eval_data_file=args.eval_data_file)
        for key, value in results.items():
            logger.info("Vailding:  %s = %s", key, round(value,4))  
        test(args, model, tokenizer, epoch=0, model_ready=True)
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            inputs = batch[0][0].to(args.device)
            prev_tokens = batch[0][1].to(args.device)
            lengths = batch[0][2].to(args.device)
            p_inputs = batch[1][0].to(args.device)
            p_prev_tokens = batch[1][1].to(args.device)
            p_lengths = batch[1][2].to(args.device)
            n_inputs = batch[2][0].to(args.device)
            n_prev_tokens = batch[2][1].to(args.device)
            n_lengths = batch[2][2].to(args.device)
            labels = batch[3].to(args.device)
            model.train()
            loss,vec = model(inputs,prev_tokens,lengths,
                             p_inputs,p_prev_tokens,p_lengths,
                             n_inputs,n_prev_tokens,n_lengths,
                             labels)
        ...
        if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
            ...
            for key, value in results.items():
                logger.info("Vailding:  %s = %s", key, round(value,4))                    
            test(args, model, tokenizer, epoch=idx+1, model_ready=True)
...
def evaluate(args, model, tokenizer,eval_when_training=False, eval_data_file=""):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            lm_loss,vec = model(inputs,prev_tokens,lengths,
                             p_inputs,p_prev_tokens,p_lengths,
                             n_inputs,n_prev_tokens,n_lengths,
                             label)
...
def test(args, model, tokenizer, epoch="best", model_ready=False):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    ...
    # multi-gpu evaluate
    if args.n_gpu > 1 and not model_ready:
        model = torch.nn.DataParallel(model)
    ...
    with open(os.path.join(args.output_dir,"predictions_epoch_{}.jsonl".format(epoch)),'w') as f:
